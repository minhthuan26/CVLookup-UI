...
import { doDeleteAccountUser, doGetAllCandidate } from '~/action/accountUserApi'
...
function CandidateManagement() {
    ...
    const getCandidateList = async (axiosPrivate, dispatch) =>
        await doGetAllCandidate(axiosPrivate, dispatch)
    const columns = [
        {
            name: 'Xem chi tiết',
            cell: (row) => (
    ...
    useEffect(
        () => {
            getCandidateList(axiosPrivate, dispatch).then((data) => {
                setCandidate(data)
                setFilter(data)
            })
        },
    ...
    useEffect(() => {
        const result = candidate.filter((item) => {
            return (
                item.user.username &&
