...
import { doAdd, doDelete, doGetById, doUpdate } from '~/action/JobFormApi'
import { toast } from 'react-toastify'
function JobFormManagement() {
    const getForm = useSelector((state) => state.jobForm.jobFormList)
    ...
    const [filter, setFilter] = useState(getForm)
    ...
    const handleDelete = (id) => {
        Confirm.open({
            ...
            onok: () => {
                try {
                    const deleteForm = async (dispatch, axiosPrivate, id) =>
                        await doDelete(dispatch, axiosPrivate, id)
                    deleteForm(dispatch, axiosPrivate, id)
                    setFilter(form.filter((item) => item.id !== id))
    ...
    const handleSave = async () => {
        try {
            if (formMode === 'add') {
                ...
                editData(dispatch, axiosPrivate, selectedForm.id, name).then(
                    (data) => {
                        setForm((prev) =>
                            prev.map((item) =>
                                item.id === data.id
...
export default JobFormManagement
